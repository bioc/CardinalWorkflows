...
```{r peak-process}
pig206_peaks <- peakProcess(pig206, SNR=3,
	sampleSize=0.1, tolerance=0.5, units="mz")

pig206_peaks
```

This produces a centroided dataset with 1,045 peaks.
...
In this case, the last 3 models (`s=16`, `s=32`, and `s=64`) all seem to be much better from the previous models, but are not significantly different from each other. However, the models for `s=16` and `s=32` support 7-8 segments, while the model for `s=64` only supports 3 segments.


### Plotting the segmentation

Let's plot the first 4 segmentations.

```{r ssc-image-multi}
image(pig206_ssc, i=3:6)
```

It is useful to see how the segmentation changes as fewer peaks are used and the number of segments decreases. Noisy, less-meaningful segments tend to be removed first, so we want to explore the segmentation with the highest value of `s` that still captures the morphology we would expect to see.
...
Here, we can see the heart, brain, and liver distinguished as segments 2, 4, and 5.
...
Let's break out the centroids for the heart, brain, and liver segments (2, 4, and 5).

```{r ssc-centers-2}
plot(pig206_ssc1, type="centers", linewidth=2,
	select=c(2,4,5), superpose=FALSE, layout=c(1,3))
```
...
Very few peaks distinguish the heart (segment 2), while many more distinguish the brain and liver (segments 4 and 5).
...
```{r top-ssc}
pig206_top <- topFeatures(pig206_ssc1)
```

Peaks associated with the heart:

```{r top-heart}
subset(pig206_top, class==2 & statistic > 0)
```

Peaks associated with the brain:

```{r top-brain}
subset(pig206_top, class==4 & statistic > 0)
```

Peaks associated with the liver:

```{r top-liver}
subset(pig206_top, class==5 & statistic > 0)
```
...
```{r cardinal-process}
cardinal_peaks <- peakProcess(cardinal, SNR=3,
	sampleSize=0.1, tolerance=0.5, units="mz")

cardinal_peaks
```

This results in a centroided dataset with 2,561 peaks.
...
cardinal_ssc <- spatialShrunkenCentroids(cardinal_peaks,
	weights="adaptive", r=2, k=8, s=2^(1:4))

cardinal_ssc
```

```{r ssc-cardinal-multi}
image(cardinal_ssc, i=1:4)
```
...
pal <- c("1"=NA, "2"=NA, "3"="darkred", "4"="firebrick",
	"5"="brown", "6"="red", "7"="black")
...
```{r top-body}
cardinal_top <- topFeatures(cardinal_ssc1)

subset(cardinal_top, class==6)

image(cardinal_peaks, mz=207.05, enhance="adaptive")
```

And let's find the *m/z* values associated with the "DESI-MS" text.

```{r top-text}
subset(cardinal_top, class==5)

image(cardinal_peaks, mz=648.79, enhance="adaptive")
```
